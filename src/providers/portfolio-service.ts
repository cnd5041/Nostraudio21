...
export class PortfolioService {
  ...
  getUserPortfolio(uid: string): void {
    ...
    //   console.log('portfolio.val', result.val());
    //   this.userPortfolio.next(result.val());
    // });
    ...
    this.af.database.object('/portfolios/' + uid)
      .subscribe((result) => {
        ...
        if (result.$exists()) {
          ...
          console.log('portfolio does not exists');
  ...
  private createPortfolio(uid: string): void {
    // TODO: See if I need to check if the user is logged in
    // Theoritically, if they made it here with a uid, they are logged in.
